//FaBo JetRacer_XIAO_ESP32S3 Version 1.2.0 アルファ版
//Board Rev3.0.4
//2024/06/19
//ESP32S3
//ブラシレスモーターセンサーを１か所計測
...
#define BOARDPATCH         4     //基板版数パッチRev3.0.4は4
...
#define BULSHLESSMOTOR_PIN1       D6  //ブラレスモーターセンサーピン１
#define BULSHLESSMOTOR_PIN2       D7  //ブラレスモーターセンサーピン２
...
void loop() {
  ...
  uint32_t rotationValue2 = pulseInLong(BULSHLESSMOTOR_PIN2, HIGH,25000);
  
  //整数からバイト列へ変換
  transfer1.before=pwm0;
  transfer2.before=pwm1;
  transfer3.before=duration;
  transfer5.before=rotationValue2;
    ...
    Serial.println(buf);
    sprintf(buf, "Sensor2 %d", rotationValue2);
    Serial.println(buf);
  #endif
